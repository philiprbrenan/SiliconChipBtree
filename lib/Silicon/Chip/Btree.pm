...
# if (my $s = $c->simulate({%i}, id=>"2024-04-19.06", pngs=>2, gsx=>2, gsy=>3, newChange=>1,  borderDx=>16, borderDy=>44, log=>1, placeFirst=>1, spaceDx=>4, spaceDy=>4, svg=>q(tree)))  #
# if (my $s = $c->simulate({%i}, id=>"2024-04-19.07", pngs=>2, gsx=>3, gsy=>2, newChange=>1,  borderDx=>16, borderDy=>44, log=>1, placeFirst=>1, spaceDx=>4, spaceDy=>4, svg=>q(tree)))  #
  if (my $s = $c->simulate({%i}, id=>"2024-04-19.08", pngs=>2, gsx=>3, gsy=>3, newChange=>1,  borderDx=>16, borderDy=>44, log=>1, placeFirst=>1, spaceDx=>4, spaceDy=>4, svg=>q(tree)))  #
   {is_deeply($s->steps,                      46);                              # Steps
    is_deeply($s->bInt($t->data),             22);                              # Data associated with search key 2
    ok($s->checkLevelsMatch);
#   is_deeply($s->length,                 300531);
