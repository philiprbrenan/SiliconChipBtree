...
#D1 B-Tree Node                                                                 # A node in a B-Tree containing keys, data and links to other nodes. Nodes only produce output when their preset id is present on their enable bus.  This makes it possible for one node to select another node for further processing of the key being found.
...
#latest:;
if (1)                                                                          #TnewBtree
 {my sub K{3} my sub L{2} my sub B{8}                                           # Maximum number of keys per node, levels in the tree, bits per key
