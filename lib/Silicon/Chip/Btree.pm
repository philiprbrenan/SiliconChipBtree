...
# if (my $s = $c->simulate({%i}, id=>"2024-04-20.12", pngs=>2, gsx=>2, gsy=>4, newChange=>1,  borderDx=>2,  borderDy=>2,  log=>1, placeFirst=>1, spaceDx=>2, spaceDy=>4, svg=>q(tree)))  #
# if (my $s = $c->simulate({%i}, id=>"2024-04-20.13", pngs=>2, gsx=>2, gsy=>4, newChange=>1,  borderDx=>1,  borderDy=>1,  log=>1, placeFirst=>1, spaceDx=>2, spaceDy=>4, svg=>q(tree)))  #
  if (my $s = $c->simulate({%i}, id=>"2024-04-20.14", pngs=>2, gsx=>2, gsy=>4, newChange=>1,  borderDx=>0,  borderDy=>0,  log=>1, placeFirst=>1, spaceDx=>2, spaceDy=>4, svg=>q(tree)))  #
   {is_deeply($s->steps,                      46);                              # Steps
    is_deeply($s->bInt($t->data),             22);                              # Data associated with search key 2
    ok($s->checkLevelsMatch);
#   is_deeply($s->length,                 300531);
